...
def Data_file():
    df = pd.read_excel('prod_out_4_180923-180924_hschoi.xlsx', sheet_name='prod_out_4')
...
def put_data_form(data_file, __opentsdb_url, metric_name):
    ...
    tag_list = ['DC_OUT_S', 'DC_OUT_M', 'WP_IN_M', 'WP_IN_S', 'WP_SPD_M', 'WP_SPD_S', 'WP_LOAD_T',
                'WP_LOAD_B', 'HS_STM', 'DISP_DIL_M', 'DISP_DIL_S', 'DISP_DIL', 'DISP_PWR_M',
                'DISP_PWR_S', 'DISP_GAP', 'HS_TMPT_M', 'HS_TMPT_S', 'DC1_LEV', 'DISP_VIB',
                'DC2_LEV', 'DISP_ENG', 'HS_TMPT_O', 'PULP_TMPT_M']
    ...
    for i in range(len(data_file)):
        ts = convertTimeToEpoch(str(data_file["datetime"].iloc[i]))
